...
for ii,Pump_val in enumerate(Pump_vals):
    ...
    effic_result=scipy.optimize.minimize(effic_fun2,x_init,args=(p),bounds=x_bounds,method='TNC',options = {'maxiter' : 100})
    print(effic_result)
    x_init=effic_result.x
    effic_result=scipy.optimize.minimize(effic_fun2,x_init,args=(p),bounds=x_bounds,method='L-BFGS-B',options = {'maxiter' : 100})
    print(effic_result)
    x_init=effic_result.x
    effic_result=scipy.optimize.minimize(effic_fun2,x_init,args=(p),bounds=x_bounds,method='TNC',options = {'maxiter' : 100})
    print(effic_result)
    ...
    np.savez('Thesis_paper/increasingpump1_N'+str(N/1e15)+'_Q'+str(Q)+'onlydeltaa',x_found=x_found,fun_found=fun_found,p=p,Pump_vals=Pump_vals)
    x_init=x_found[:,ii]

plt.plot(Pump_vals,fun_found)
plt.title('N'+str(N/1e15)+'_Q'+str(Q)+'onlydeltaa_small')
plt.show()
