...
delta3val_start=-0.7e10
delta2val_start=-0.7e8 # T=500e-3
...
N_frac=(np.exp((6.63e-34*p['freqmu'])/(1.38e-23*temp_vals[0]))-1)/(np.exp((6.63e-34*p['freqmu'])/(1.38e-23*temp_vals[0]))+1)
if np.isnan(N_frac):
    N_frac=1
deltamuc_start=p['Nm']*p['gm']**2/delta2val_start*N_frac
deltaoc_start=p['No']*p['go']**2/delta3val_start
...
print(effic_fun2([-2.2,-0.7],temp_val,p))
...
for ii,temp_val in enumerate(temp_vals):
    ...
    np.savez('Thesis_paper/increasingtemp1_N'+str(N/1e15)+'_Q'+str(Q)+'_Pump='+str(Pump_val)+'4',x_found=x_found,fun_found=fun_found,p=p,temp_vals=temp_vals)
    x_init=x_found[:,ii]

plt.plot(temp_vals,fun_found)
plt.title('N'+str(N/1e15)+'_Q'+str(Q)+'_Pump='+str(Pump_val))
plt.show()
