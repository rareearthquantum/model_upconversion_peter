...
delta2vals=np.linspace(-2e8,0e8,31)[:-1]
delta3vals=np.linspace(-8e10,-0e10,31)[:-1]/4
P_pump_mW=1
# P_pump_mW=1
...
N_frac1=np.zeros((len(delta2vals),len(delta3vals)),dtype=np.complex_)
...
for ii,delta2val in enumerate(delta2vals):
    ...
    deltamucval=p['Nm']*p['gm']**2/delta2val*N_frac
    p['nbath']=nbath_from_T(T,p)
    for jj, delta3val in enumerate(delta3vals):
        ...
        #time1=time.time()
        Caa[ii,jj], Cba[ii,jj], Cab[ii,jj], Cbb[ii,jj],N_frac1[ii,jj]=calc_efficiency(deltaoval,deltamval,deltaocval,deltamucval,p)
