...
def rho_m_broad_full(double delaoval,double complex aval, double complex bval,double deloval,double delmval, p):
    cdef double points[18]#[6]
    cdef double S_out[9]
    cdef double S_temp[9]
    cdef double pointpairs[2]
    cdef double omegamu = abs(p['gm']*bval)
    ...
    ,delmval-2*p['gamma2d'],delaoval-deloval+delmval, #,p['sd_delam']+p['mean_delam']]
    -omegamu2/(delaoval-deloval)*2+delmval,(delaoval-deloval)/2+sqrt((delaoval-deloval)**2/4-omegamu2)+delmval,(delaoval-deloval)/2-sqrt((delaoval-deloval)**2/4-omegamu2)+delmval,
    -omegamu2/(delaoval-deloval)*2+delmval-omegamu,(delaoval-deloval)/2+sqrt((delaoval-deloval)**2/4-omegamu2)+delmval-omegamu,(delaoval-deloval)/2-sqrt((delaoval-deloval)**2/4-omegamu2)+delmval-omegamu,
    -omegamu2/(delaoval-deloval)*2+delmval+omegamu,(delaoval-deloval)/2+sqrt((delaoval-deloval)**2/4-omegamu2)+delmval+omegamu,(delaoval-deloval)/2-sqrt((delaoval-deloval)**2/4-omegamu2)+delmval+omegamu])
