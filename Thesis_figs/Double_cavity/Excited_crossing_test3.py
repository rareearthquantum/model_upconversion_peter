...
filename='Thesis_figs/Double_cavity/Excited_crossing3_test_'
B_vals=np.linspace(0.226,0.238,101)
deltamvals=np.linspace(-30e6,30e6,101)*2*np.pi
# B_vals=np.linspace(0.226,0.238,21)
# deltamvals=np.linspace(0e6,25e6,3)*2*np.pi
...
#['No'] = p['Nm'] # number of atoms in the optical mode
...
def deltaao_from_I(B_mag,p):
    return -(2*pi*(p['freqmu']+p['freq_pump'])-omegaao4_from_I(B_mag,p))
...
def fields_vec_fun(fields_vec,ainval,bval,deloval,delmval,p):
    ...
    # p['Omega']=Omega
    # S12val=rho[2,1]*p['No']*p['gm']
    S13val=rho[0,2]*p['No']*p['go']
    # S21_onlyb_val=rho_only_b[2,1]*(p['Nm']-p['No'])*p['gm']
    ...
    return [aval1.real,aval1.imag]-fields_vec
def fields_abs_fun(fields_vec,ainval,bval,deloval,delmval,p):
    aval=fields_vec[0]+1j*fields_vec[1]
    Omega=p['Omega']
    rho=np.array(rho_broad_full(aval,bval,deloval,delmval,p))
    # p['Omega']=0
    ...
    #bval1=(-1j*S12val+np.sqrt(p['gammamc'])*binval)/((p['gammamc']+p['gammami'])/2-1j*delmval)
    aval1=(-1j*S13val+np.sqrt(p['gammaoc'])*ainval)/((p['gammaoc']+p['gammaoi'])/2-1j*deloval)
    out_vec=np.array([aval1.real,aval1.imag,]-fields_vec)
    out_abs=np.linalg.norm(out_vec)
    return out_abs

def find_fields(ainval,deloval,delmval,start_guess_vec,p):
    ...
    start_guess_vec_a=[start_guess_vec[0],start_guess_vec[1]]
    bval=start_guess_vec[2]+1j*start_guess_vec[3]
    fields_found=scipy.optimize.root(fields_vec_fun,start_guess_vec_a,args=(ainval,bval,deloval,delmval,p),method='hybr')
    ...
    if not fields_found.success:
        ...
        fields_found=scipy.optimize.minimize(fields_abs_fun,start_guess_vec_a,args=(ainval,binval,deloval,delmval,p),bounds=minimize_bounds)
    if not fields_found.success:
        print(fields_found.message)
...
for ii, B_val in enumerate(B_vals):
    ...
    p['sd_delao']=sd_delao4_from_I(B_vals[0],p)
    p['Omega']= Omega_cavity_from_PdBm_resonance(P_pump,p)
    ...
    for jj, deltamval in enumerate(deltamvals):
        ...
        avals[ii,jj] = find_fields(ainval,deltaoval,deltamval,start_guess_vec,p)
