...
import time
from c_funs_excited_ds import steady_rho_single_c, gauss_fun_1d
...
p['gm'] = 1.04/100 #coupling between atoms and microwave field
p['T']=T
p['Wbeam']=0.6e-3
...
p['nYSO'] = 1.76 #refractive index of YSO#refractive index of YSO
...
# npzdata=np.load('Documents/ltj-CaltechMeasurements/Thesis_matlab_figs/TB/TB_excited2/test_crossing_excited4_TB2_sd2_BigpumpT=0.15Pmu=-30.1.npz')
npzdata=np.load('test_crossing_excited4_TB2_sd2_BigpumpT=0.15Pmu=-30.1.npz')
...
def find_dressed_states_m_a(delaoval, deloval,delmval,aval,bval,p):
    try:
        deltam_ds_val=scipy.optimize.fsolve((H_disc_diff_a_fun),np.array((p['gm']*bval)**2/(delaoval-deloval)),args=(delaoval-deloval,aval,bval,p['go'],p['gm'],p['Omega']))+delmval
    except ValueError:
...
def find_dressed_states_m_a2(delaoval, deloval,delmval,aval,bval,p):
    try:
        deltam_ds_val=scipy.optimize.fsolve((H_disc_diff_a_fun),np.array(-(p['Omega'])**2/(delaoval-deloval)+delaoval-deloval),args=(delaoval-deloval,aval,bval,p['go'],p['gm'],p['Omega']))+delmval
    except ValueError:
        deltam_ds_val=np.nan
    except TypeError:
        deltam_ds_val=np.nan
...
def lines_m(delaovals,deloval,delmval,aval,bval,p):
    ...
    for ii, delaoval in enumerate(delaovals):
        ...
        mlines[:,ii]=np.array([ ds_m_val_a1,ds_m_val_a2,(p['gm']*bval)**2/(delaoval-deloval)+delmval,-(p['Omega'])**2/(delaoval-deloval)+delaoval-deloval+delmval])
        #mlines[abs(mlines)>max(delaovals)]=np.nan
    return mlines.T
#p['Omega']=1000
# p['Omega']= Omega_from_PdBm_cavity(P_pump,p)
# p['Omega']= Omega_from_PdBm(P_pump/10,p)

# bval=bin_from_PdBm(P_mu,p)/np.sqrt(p['gammamc'])*1000

bval=1.5e5
aval=bval*1e-8*0
delamvals=np.linspace(-1,1,101)*1e7*2
delaovals=np.linspace(-1,1,101)*1e7*2
delm=-1.e7*0
delo=2.e6*0
#
# bval=10#bin_from_PdBm(P_mu,p)/np.sqrt(p['gammamc'])
# aval=1.0
# delamvals=np.linspace(-1,1,51)*1e7*2
# delaovals=np.linspace(-1,1,51)*1e7*2
# delm=-1.e6*0
# delo=2.e6*0
...
filename='rho_excited_bigb4_'
filename='rho_excited_TEST'
filename='rho_excited_b'+str(bval)
...
img1=ax.imshow((np.abs(rhovals[1,2,:,:])),extent=(min(delamvals),max(delamvals),min(delaovals),max(delaovals)),aspect='auto',origin='lower')
plt.plot(mlines[:,:],delaovals1,'-',alpha=0.5,linewidth=1)
