...
for ii,Pump_val in enumerate(Pump_vals):
    ...
    # x_init= [p['No']*p['go']**2/-1.6e10*1e-10,p['Nm']*p['gm']**2/-0.67e8*1e-8,-1.9,-0.6,2*pi*1.7,2*pi*1.495]
    x_init= [p['No']*p['go']**2/(delta3_start[ii]*1e10)*1e-10,N_frac*p['Nm']*p['gm']**2/(delta2_start[ii]*1e8)*1e-8,delta3_start[ii],delta2_start[ii],2*pi*1.7,2*pi*1.495]
    print(Pump_val)
...
np.savez('Thesis_figs/Linear_effic/increasingpump_test1',x_found=x_found,fun_found=fun_found,p=p,Pump_vals=Pump_vals)
