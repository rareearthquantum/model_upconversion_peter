...
def effic_fun6big(x, Pump_val,p):
#x=[deloval,delmval,delocval,delmucval,mean_delao,mean_delao,gammaoc]
    # deloval=x[0]*1e10
    # delmval=x[1]*1e8
    deloval=0
...
def effic_fun2_gamma(x,Pump_val, p):
#x=[deloval,delmval,delocval,delmucval,mean_delao,mean_delao,gammaoc]
    # deloval=x[0]*1e10
    # delmval=x[1]*1e8
    deloval=0
...
Pump_vals=[1,5,10,20,50,100,1000]
Pump_vals=[2,3,7,15,30,75,150,200,500]
...
for ii,Pump_val in enumerate(Pump_vals):
    ...
    #         effic_result2=scipy.optimize.minimize(effic_fun2,x_init,args=(p),method='TNC',bounds=x_bounds2,options = {'maxiter' : 400})
    #         print(effic_fun2(effic_result2.x,p))
    #         print(effic_result2)
    #         if not np.isnan(effic_result2.fun):
    #             x_init=effic_result2.x
    ...
    if ii!=0:
        effic_x1= effic_fun2_gamma(x_found_gamma[:,ii-1],Pump_val,p)
        effic_x2= effic_fun2_gamma(x_init_gamma,Pump_val,p)
        if effic_x1<effic_x2:
            x_init_gamma=x_found_gamma[:,ii-1]
    x_init=x_init_gamma
    p['mean_delao']=effic_result2.x[0]*1e10
    p['mean_delam']=effic_result2.x[1]*1e8
    effic_result2gamma=scipy.optimize.minimize(effic_fun2_gamma,x_init_gamma,args=(Pump_val,p),bounds=x_bounds_gamma)
    print(effic_result2gamma)
    if not np.isnan(effic_result2gamma.fun):
        x_init=effic_result2gamma.x
    print('=======')
    effic_result2gamma=scipy.optimize.minimize(effic_fun2_gamma,x_init,args=(Pump_val,p),method='TNC',bounds=x_bounds_gamma,options = {'maxiter' : 200})
    print(effic_result2gamma)
    if not np.isnan(effic_result2gamma.fun):
        x_init=effic_result2gamma.x
    # print('=======')
    # effic_result2gamma=scipy.optimize.minimize(effic_fun2_gamma,x_init,args=(Pump_val,p),method='L-BFGS-B',bounds=x_bounds_gamma)
    # print(effic_fun2_gamma(effic_result2gamma.x,p))
    # print(effic_result2gamma)
    # if not np.isnan(effic_result2gamma.fun):
    #     x_init=effic_result2gamma.x
    # print('=======')
    # effic_result2gamma=scipy.optimize.minimize(effic_fun2_gamma,x_init,args=(Pump_val,p),method='TNC',bounds=x_bounds_gamma)
    # print(effic_fun2_gamma(effic_result2gamma.x,p))
    # print(effic_result2gamma)
    # print('=======')
    # for jj in range(6):
    #     if not effic_result2gamma.success:
    #         effic_result2gamma=scipy.optimize.minimize(effic_fun2_gamma,x_init,args=(Pump_val,p),method='TNC',bounds=x_bounds_gamma,options = {'maxiter' : 400})
    #         print(effic_fun2_gamma(effic_result2gamma.x,p))
    #         print(effic_result2gamma)
    #         if not np.isnan(effic_result2gamma.fun):
    #             x_init=effic_result2gamma.x
    ...
    x_init6= [p['No']*p['go']**2/(x_found2[0,ii]*1e10)*1e-10,N_frac*p['Nm']*p['gm']**2/(x_found2[1,ii]*1e8)*1e-8,x_found2[0,ii],x_found2[1,ii],x_found_gamma[0,ii],x_found_gamma[1,ii]]
    x_init=x_init6
    effic_result6=scipy.optimize.minimize(effic_fun6,x_init6,args=(Pump_val,p),bounds=x_bounds6)
    print(effic_result6)
    if not np.isnan(effic_result6.fun):
        x_init=effic_result6.x
    effic_result6=scipy.optimize.minimize(effic_fun6big,x_init6,args=(Pump_val,p),bounds=x_bounds6)
    print(effic_result6)
    if not np.isnan(effic_result6.fun):
        x_init=effic_result6.x
    effic_result6=scipy.optimize.minimize(effic_fun6big,x_init,args=(Pump_val,p),bounds=x_bounds6,method='TNC',options = {'maxiter' : 400})
    print(effic_result6)
    if not np.isnan(effic_result6.fun):
        x_init=effic_result6.x
    effic_result6=scipy.optimize.minimize(effic_fun6big,x_init,args=(Pump_val,p),bounds=x_bounds6,method='L-BFGS-B',options = {'maxiter' : 200})
    print(effic_result6)
    if not np.isnan(effic_result6.fun):
        x_init=effic_result6.x
    effic_result6=scipy.optimize.minimize(effic_fun6big,x_init,args=(Pump_val,p),bounds=x_bounds6,method='TNC',options = {'maxiter' : 400})
    ...
    np.savez('Thesis_figs/Linear_effic/increasingpump3test_test2differentnums_N'+str(N/1e15)+'',x_found6=x_found6,fun_found6=fun_found6,p=p,Pump_vals=Pump_vals,
