...
deltamvals=np.linspace(-50e6,50e6,41)*2*np.pi

# deltamvals=np.linspace(-15e6,15e6,41)*2*np.pi
# deltamvals=np.linspace(-35e6,35e6,21)*2*np.pi

Omegavals=np.linspace(Omega_cavity_from_PdBm_resonance(10*np.log10(1.74),p),Omega_cavity_from_PdBm_resonance(10*np.log10(1000.74),p),6)#*1e7#bin_from_PdBm(P_mu,p)
B_val=0.2115
# B_val=0.21165
P_mu = -60 # in dBm
ainval=0
...
filename='Thesis_figs/Twiddling_parameters/splitting_Omega_T='+str(T)+'Nm=/=No_highT,Bval='+str(B_val)
...
bvals=np.zeros((len(Omegavals),len(deltamvals)),dtype=np.complex_)
avals=np.zeros((len(Omegavals),len(deltamvals)),dtype=np.complex_)
rho_out=np.zeros((3,3,len(Omegavals),len(deltamvals)),dtype=np.complex_)
avals=np.zeros((len(Omegavals),len(deltamvals)),dtype=np.complex_)
binvals=np.zeros((len(Omegavals),len(deltamvals)),dtype=np.complex_)
calc_time=np.zeros((len(Omegavals),len(deltamvals)))

start_time=time.time()


for ii, Omegaval in enumerate(Omegavals):
    ...
    p['Omega']= Omegaval#Omega_cavity_from_PdBm_resonance(P_pump,p)



    for jj, deltamval in enumerate(deltamvals):

        p['mean_delao']=0#deltaao_from_B(B_val,deltamval,p)
        time1=time.time()
    ...
    #np.savez(filename,binvals=binvals,B_vals=B_vals,deltamvals=deltamvals,p=p,bvals=bvals,T_vals=T_vals,rho_out=rho_out,P_pump=P_pump,P_mu=P_mu,T=T,elapsed_time=elapsed_time,avals=avals,deltaoval=deltaoval,calc_time=calc_time)
print('    ===========================Complete===========================')
...
img1=ax.imshow(10*np.log10(np.abs(aoutvals/binvals)**2),extent=(np.min(freqmu_vals),np.max(freqmu_vals),np.min(Omegavals),np.max(Omegavals)),aspect='auto',origin='lower')
...
img1=ax.imshow(10*np.log10(np.abs(boutvals/binvals)**2),extent=(np.min(freqmu_vals),np.max(freqmu_vals),np.min(Omegavals),np.max(Omegavals)),aspect='auto',origin='lower')
...
img1=ax.imshow((np.abs(aoutvals/binvals)**2),extent=(np.min(freqmu_vals),np.max(freqmu_vals),np.min(Omegavals),np.max(Omegavals)),aspect='auto',origin='lower')
plt.title('|aout/bin|')
plt.xlabel('delta_mu')
plt.ylabel('I')
...
img1=ax.imshow((np.abs(boutvals/binvals)**2),extent=(np.min(freqmu_vals),np.max(freqmu_vals),np.min(Omegavals),np.max(Omegavals)),aspect='auto',origin='lower')
...
img1=ax.imshow((np.angle(aoutvals)),extent=(np.min(freqmu_vals),np.max(freqmu_vals),np.min(Omegavals),np.max(Omegavals)),aspect='auto',origin='lower',cmap='hsv')
plt.title(' rho13 phase')
plt.xlabel('delta_mu')
plt.ylabel('I')
fig.colorbar(img1)
ax=fig.add_subplot(3,2,6)
img1=ax.imshow((np.angle(boutvals[:,:])),extent=(np.min(freqmu_vals),np.max(freqmu_vals),np.min(Omegavals),np.max(Omegavals)),aspect='auto',origin='lower',cmap='hsv')
...
for ii in range(3):
    for jj in range(3):
        ...
        img1=ax.imshow((np.abs(rho_out[ii,jj,:,:])),extent=(np.min(freqmu_vals),np.max(freqmu_vals),np.min(Omegavals),np.max(Omegavals)),aspect='auto',origin='lower')
