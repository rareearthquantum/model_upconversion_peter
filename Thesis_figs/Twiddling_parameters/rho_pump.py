...
P_pump_vals_mW=np.linspace(0,1000,2001)
# P_pump_vals_mW=np.linspace(0,2e15,201)
# P_pump_vals_mW=np.logspace(0,18,2001)

P_pump_vals_dBm=10*np.log10(P_pump_vals_mW)
rho_out=np.zeros((3,3,len(P_pump_vals_mW)),dtype=np.complex_)
Omega_vals=np.zeros(len(P_pump_vals_mW))
TT=150e-3
aval=bval*1e-4*0
B_val=0.205
filename='Thesis_figs/Twiddling_parameters/rho_pump1'
for ii, P_pump_val_dBm in enumerate(P_pump_vals_dBm):
    ...
    # p['Omega']= Omega_cavity_from_PdBm_resonance(P_pump_val_dBm,p)
    p['Omega']= Omega_from_PdBm(P_pump_val_dBm,p)

    Omega_vals[ii]=p['Omega']
    deltaoval=0
    deltamval=0
    rho_out[:,:,ii]=rho_broad_full(aval,bval,deltaoval,deltamval,p)

np.savez(filename,rho_out=rho_out,TT=TT,aval=aval,bval=bval,p=p,B_val=B_val,deltamval=deltamval,deltaoval=deltaoval,
        Omega_vals=Omega_vals,P_pump_vals_dBm=P_pump_vals_dBm,P_pump_vals_mW=P_pump_vals_mW)

fig=plt.figure('rho with pump power',figsize=(12,12))
...
for ii in range(3):
    for jj in range(3):
        ...
        plt.plot(P_pump_vals_mW,abs(rho_out[ii,jj,:]))
...
fig=plt.figure('rho/P_pump',figsize=(12,12))
...
for ii in range(3):
    for jj in range(3):
        ...
        plt.plot(P_pump_vals_mW,abs(rho_out[ii,jj,:])/P_pump_vals_mW)
...
fig=plt.figure('rho/Omega',figsize=(12,12))
...
for ii in range(3):
    for jj in range(3):
        ...
        plt.plot(P_pump_vals_mW,abs(rho_out[ii,jj,:])/Omega_vals)
...
plt.figure()
plt.plot(P_pump_vals_mW,rho_out[0,0,:]-rho_out[1,1,:])
plt.show()
