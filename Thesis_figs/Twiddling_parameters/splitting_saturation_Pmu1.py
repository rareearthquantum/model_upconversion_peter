...
deltamvals=np.linspace(-100e6,100e6,51)*2*np.pi
deltamvals=np.linspace(-30e6,30e6,51)*2*np.pi
deltamvals=np.linspace(-15e6,15e6,51)*2*np.pi

P_mu_vals=np.linspace(-70,-50,11)
# P_mu_vals=np.linspace(-50,-10,11)
...
filename='Thesis_figs/Twiddling_parameters/splitting_saturation_Pmu3_small_B='+str(B_val)+',T='+str(T)
if p['Nm']==p['No']:
    filename=filename+'sameN'
else:
    filename=filename+'diffN'
print(filename)
...
bvals=np.zeros((len(P_mu_vals),len(deltamvals)),dtype=np.complex_)
avals=np.zeros((len(P_mu_vals),len(deltamvals)),dtype=np.complex_)
rho_out=np.zeros((3,3,len(P_mu_vals),len(deltamvals)),dtype=np.complex_)
avals=np.zeros((len(P_mu_vals),len(deltamvals)),dtype=np.complex_)
binvals=np.zeros((len(P_mu_vals),len(deltamvals)),dtype=np.complex_)
calc_time=np.zeros((len(P_mu_vals),len(deltamvals)))

start_time=time.time()


for ii, P_mu_val in enumerate(P_mu_vals):
    ...
    for jj, deltamval in enumerate(deltamvals):
        binval=bin_from_PdBm(P_mu_val,p)
    ...
    np.savez(filename,P_mu_vals=P_mu_vals,binvals=binvals,B_val=B_val,deltamvals=deltamvals,p=p,bvals=bvals,rho_out=rho_out,P_pump=P_pump,T=T,elapsed_time=elapsed_time,avals=avals,deltaoval=deltaoval,calc_time=calc_time,ii=ii,jj=jj)
print('    ===========================Complete===========================')
...
img1=ax.imshow(10*np.log10(np.abs(aoutvals/binvals)**2),extent=(np.min(freqmu_vals),np.max(freqmu_vals),np.min(P_mu_vals),np.max(P_mu_vals)),aspect='auto',origin='lower')
...
img1=ax.imshow(10*np.log10(np.abs(boutvals/binvals)**2),extent=(np.min(freqmu_vals),np.max(freqmu_vals),np.min(P_mu_vals),np.max(P_mu_vals)),aspect='auto',origin='lower')
...
img1=ax.imshow((np.abs(aoutvals/binvals)**2),extent=(np.min(freqmu_vals),np.max(freqmu_vals),np.min(P_mu_vals),np.max(P_mu_vals)),aspect='auto',origin='lower')
plt.title('|aout/bin|')
plt.xlabel('delta_mu')
plt.ylabel('I')
...
img1=ax.imshow((np.abs(boutvals/binvals)**2),extent=(np.min(freqmu_vals),np.max(freqmu_vals),np.min(P_mu_vals),np.max(P_mu_vals)),aspect='auto',origin='lower')
...
img1=ax.imshow((np.angle(aoutvals)),extent=(np.min(freqmu_vals),np.max(freqmu_vals),np.min(P_mu_vals),np.max(P_mu_vals)),aspect='auto',origin='lower',cmap='hsv')
plt.title(' rho13 phase')
plt.xlabel('delta_mu')
plt.ylabel('I')
fig.colorbar(img1)
ax=fig.add_subplot(3,2,6)
img1=ax.imshow((np.angle(boutvals[:,:])),extent=(np.min(freqmu_vals),np.max(freqmu_vals),np.min(P_mu_vals),np.max(P_mu_vals)),aspect='auto',origin='lower',cmap='hsv')
...
for ii in range(3):
    for jj in range(3):
        ...
        img1=ax.imshow((np.abs(rho_out[ii,jj,:,:])),extent=(np.min(freqmu_vals),np.max(freqmu_vals),np.min(P_mu_vals),np.max(P_mu_vals)),aspect='auto',origin='lower')
