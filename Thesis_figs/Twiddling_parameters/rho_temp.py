...
aval=bval*1e-5
T_vals=np.linspace(0,1000,2000)*1e-3
rho_out=np.zeros((3,3,len(T_vals)),dtype=np.complex_)

B_val=0.2115
for ii, TT in enumerate(T_vals):
...
# np.savez('Thesis_figs/Twiddling_parameters/rho_temp1',p=p,B_val=B_val,rho_out=rho_out,T_vals=T_vals,bval=bval,aval=aval)
...
for ii in range(3):
    for jj in range(3):
        ...
        plt.plot(T_vals,abs(rho_out[ii,jj,:]))
...
for ii in range(3):
    for jj in range(3):
        ...
        plt.plot(T_vals,np.angle(rho_out[ii,jj,:]))
...
plt.figure()
plt.plot(T_vals,rho_out[0,0,:]-rho_out[1,1,:])
