...
filename='Thesis_figs/Double_cavity/Ground_state_crossing2_test1_small_500_mK_sameN'
B_vals=np.linspace(0.205,0.215,31)
deltamvals=np.linspace(-100e6,100e6,31)*2*np.pi
B_vals=np.linspace(0.205,0.220,61)
...
filename='Thesis_figs/ain/Ground_state_crossing_ain_test_fast'+ 'ain=' +str(ainval)+ 'T='+str(T) +'diffN'
...
for ii, B_val in enumerate(B_vals):
    ...
    for jj, deltamval in enumerate(deltamvals):
        ...
        time1=time.time()
        binval=0#bin_from_PdBm(P_mu,p,deltamval)
