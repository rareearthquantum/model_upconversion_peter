...
T=100e-3
from Thesis_figs.Ground_params import p, sd_delao_from_B, deltaao_from_B
...
delta2vals=np.linspace(-200e6,200e6,11)*10/1
deltamuvals=np.linspace(-200e6,200e6,11)*10/1
deltaocval=0e10
deltamucval=0e8
deloval=-0e10#0e10
...
p['mean_delao']=deltaao_from_B(0,0,p)
...
fig=plt.figure(filename+'effic_full')

fig.clf()

ax=fig.add_subplot(2,1,1)
#img1=ax.imshow(10*np.log10(np.abs(effic_abia)/np.abs(Cba**2)),extent=(np.min(deltamuvals),np.max(deltamuvals),np.min(delta2vals),np.max(delta2vals)),aspect='auto',origin='lower',cmap='RdBu')
img1=ax.imshow(np.log10(np.abs(effic_full)/np.abs(Cba**2)),extent=(np.min(deltamuvals),np.max(deltamuvals),np.min(delta2vals),np.max(delta2vals)),aspect='auto',origin='lower',cmap='RdBu',norm=MidpointNormalize(midpoint=0))
#img1=ax.imshow(np.abs(10*np.log10(np.abs(effic_abia)/np.abs(Cba**2))),extent=(np.min(deltamuvals),np.max(deltamuvals),np.min(delta2vals),np.max(delta2vals)),aspect='auto',origin='lower')
#img1=ax.imshow(10*np.log10(np.abs(effic_abia)/np.abs(Cba**2)),extent=(np.min(deltamuvals),np.max(deltamuvals),np.min(delta2vals),np.max(delta2vals)),aspect='auto',origin='lower')
#plt.plot(p['No']*p['gm']**2/delta2vals2,delta2vals2)
plt.title(' |C|^2 (dB scale)')
...
img1=ax.imshow((np.abs(effic_full)/np.abs(Cba**2)),extent=(np.min(deltamuvals),np.max(deltamuvals),np.min(delta2vals),np.max(delta2vals)),aspect='auto',origin='lower')
plt.title('|C|^2 ')
plt.ylabel('atom delta')
plt.xlabel('Cavity delta')
fig.colorbar(img1, format='%.0e')
...
fig.suptitle('effic_full/effic_linear')
