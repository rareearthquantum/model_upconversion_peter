...
T=50e-3
P_mu=-200
filename='compare3_test_zoomout2'
filename=filename+'T = '+ str(T) + ', P_mu = ' + str(P_mu) +'N='+str(N/1e15)
...
for ii,delta2val in enumerate(delta2vals):
    ...
    for jj, delmval in enumerate(deltamuvals):
        ...
        binval=bin_from_PdBm(P_mu,p,delmval)#1/np.sqrt(p['gammamc'])#
        ...
        avals[ii,jj], bvals[ii,jj] ,rho_out[:,:,ii,jj]= find_fields(ainval,binval,deloval,delmval,start_guess_vec,p)
    ...
    np.savez(save_dir+filename,P_pump=P_pump,T=T,P_mu=P_mu,p=p,binvals=binvals,ainval=ainval,avals=avals,bvals=bvals, rho_out=rho_out,deloval=deloval,deltaocval=deltaocval,
            ...
            delta2vals=delta2vals,elapsed_time=elapsed_time)
print('    ===========================Complete===========================')
