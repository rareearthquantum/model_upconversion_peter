...
filename='Thesis_figs/Cavity_shifts/optical_linear_shift2'
#I_vals=np.linspace(7.43,7.55,31)
delta3vals=np.linspace(-390e8,390e8,25)*2*np.pi
deltaocvals=np.linspace(-5e8,5e8,75)
...
img1=ax.imshow((np.angle(Caa)),extent=(np.min(deltaocvals),np.max(deltaocvals),np.min(delta3vals),np.max(delta3vals)),aspect='auto',origin='lower',cmap='hsv',clim=(-np.pi,np.pi))
plt.title(' rho13 phase')
plt.xlabel('delta_mu')
plt.ylabel('I')
fig.colorbar(img1)
ax=fig.add_subplot(3,2,6)
img1=ax.imshow((np.angle(Cab)),extent=(np.min(deltaocvals),np.max(deltaocvals),np.min(delta3vals),np.max(delta3vals)),aspect='auto',origin='lower',cmap='hsv',clim=(-np.pi,np.pi))
