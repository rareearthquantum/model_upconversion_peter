...
delovals=np.linspace(0,p['mean_delao']/2,19) #2,4
#delovals=np.linspace(p['mean_delao'], p['mean_delao'],19) #1,3

#delmvals=np.linspace(0,p['mean_delam']/2,19) #3,4
delmvals=np.linspace(p['mean_delam']/2,p['mean_delam'],19) #1,2
binvals=[600000]
ainvals=[600000]
aoutvals,boutvals,effic_a,effic_b=find_outputs(ainvals,binvals,delovals,delmvals,p)
np.savez('output_N20_d_ainbin2',aoutvals=aoutvals,boutvals=boutvals,effic_a=effic_a,effic_b=effic_b,ainvals=ainvals,binvals=binvals,delovals=delovals,delmvals=delmvals,p=p)
