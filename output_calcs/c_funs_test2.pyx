...
def rho_m_broad_single(double delaoval,double complex aval, double complex bval, double deloval,double delmval, p,delamlims):
    ...
    for ii in range(n_gauss):
        yval=(delamlims[1]-delamlims[0])*(xgauss[ii]+1)/2.0 + delamlims[0]
...
def rho_m_broad_full(double delaoval,double complex aval, double complex bval,double deloval,double delmval, p):
    ...
    cdef int ii,jj
    S_out=[0,0,0, 0,0,0, 0,0,0]
    points[:]=sorted([-100*p['sd_delam']+p['mean_delam'],100*p['sd_delam']+p['mean_delam'],p['mean_delam'],p['sd_delam']+p['mean_delam'] ,-p['sd_delam']+p['mean_delam'],delmval,delmval+2*p['gamma2d'],delmval-2*p['gamma2d']])#,p['sd_delam']+p['mean_delam']]
    #print(points)
    for ii in range(7):
        S_temp=rho_m_broad_single(delaoval,aval,bval,deloval,delmval, p,[points[ii], points[ii+1]])
        #print(S_temp)
...
def rho_broad_full(double complex aval,double complex bval,double deloval,double delmval, p):
    ...
    points[:]=sorted([-100*p['sd_delao']+p['mean_delao'],100*p['sd_delao']+p['mean_delao'],p['mean_delao'],-3*p['sd_delao']+p['mean_delao'],3*p['sd_delao']+p['mean_delao'],-p['sd_delao']+p['mean_delao'],p['sd_delao']+p['mean_delao'],deloval,deloval+2*p['gamma3d'],deloval-2*p['gamma3d']])
    #points.sort()

    for ii in range(9):
        S_temp=rho_o_broad_single(aval,bval,deloval,delmval, p,[points[ii], points[ii+1]])
