...
import c_linear_ground3
import time
delaovals=np.linspace(-1e9,1e9,5)
delamvals=np.linspace(-1e9,1e9,5)
p = {}
p['nbath']=20
...
rho03=np.zeros((3,3,len(delaovals),len(delamvals)),dtype=np.complex_)
rhoa3=np.zeros((3,3,len(delaovals),len(delamvals)),dtype=np.complex_)
rhoac3=np.zeros((3,3,len(delaovals),len(delamvals)),dtype=np.complex_)
rhob3=np.zeros((3,3,len(delaovals),len(delamvals)),dtype=np.complex_)
rhobc3=np.zeros((3,3,len(delaovals),len(delamvals)),dtype=np.complex_)
...
for ii, deloval in enumerate(delaovals):
    ...
    for jj, delmval in enumerate(delamvals):
        rho0[:,:,ii,jj],rhoa[:,:,ii,jj],rhoac[:,:,ii,jj],rhob[:,:,ii,jj],rhobc[:,:,ii,jj]=c_linear_ground4.rho_broad_full(deloval,delmval, 0,p)
        #print(time.time()-t1)
        t1tot=time.time()-t1
print(t1tot)
...
for ii, deloval in enumerate(delaovals):
    #print(ii)
    for jj, delmval in enumerate(delamvals):
        rho03[:,:,ii,jj],rhoa3[:,:,ii,jj],rhoac3[:,:,ii,jj],rhob3[:,:,ii,jj],rhobc3[:,:,ii,jj]=c_linear_ground3.rho_broad_full(deloval,delmval, 0,p)
        #print(time.time()-t1)
        t2tot=time.time()-t1
...
for kk, rhox in enumerate([rho0,rhoa,rhoac,rhob, rhobc]):
    ...
    for ii in range(3):
        for jj in range(3):
            ...
            fig.colorbar(img1)
            pltnum=pltnum+1
for kk, rhox in enumerate([rho03,rhoa3,rhoac3,rhob3, rhobc3]):
    fig=plt.figure(titles[kk]+' 3')
    pltnum=1
    fig.clf()
    for ii in range(3):
        for jj in range(3):
...
rhoxs=[rho0,rhoa,rhoac,rhob, rhobc]
for kk, rhox3 in enumerate([rho03,rhoa3,rhoac3,rhob3, rhobc3]):
    rhox=rhoxs[kk]
    fig=plt.figure(titles[kk]+' diff')
    pltnum=1
    fig.clf()
    for ii in range(3):
        for jj in range(3):
            ...
            ax=fig.add_subplot(3,3,pltnum)
            img1=ax.imshow((np.abs(rhox3[ii,jj,:,:]-rhox[ii,jj,:,:])),extent=(min(delamvals),max(delamvals),min(delaovals),max(delaovals)),aspect='auto',origin='lower')
