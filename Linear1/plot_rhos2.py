...
import time
delaovals=np.linspace(-1e9,1e9,21)
delamvals=np.linspace(-1e9,1e9,21)
p = {}
p['nbath']=20
...
for ii, deloval in enumerate(delaovals):
    ...
    for jj, delmval in enumerate(delamvals):
        rho0[:,:,ii,jj],rhoa[:,:,ii,jj],rhoac[:,:,ii,jj],rhob[:,:,ii,jj],rhobc[:,:,ii,jj]=c_linear_ground.rho_broad_full(deloval,delmval, 0,p)
        #print(time.time()-t1)
        t1tot=time.time()-t1
print(t1tot)
