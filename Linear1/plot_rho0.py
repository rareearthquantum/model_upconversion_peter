...
import c_linear_ground2

delaovals=np.linspace(-1e9,1e9,10)
delamvals=np.linspace(-1e9,1e9,10)
p = {}
p['nbath']=10
...
rho0_single1=np.zeros((3,3,len(delaovals),len(delamvals)),dtype=np.complex_)
rho0_single2=np.zeros((3,3,len(delaovals),len(delamvals)),dtype=np.complex_)
...
for ii, deloval in enumerate(delaovals):
    ...
    for jj, delmval in enumerate(delamvals):
        V=c_linear_ground1.steady_rhos(deloval,delmval,0, 0,p)
        rho0_single1[:,:,ii,jj]=np.array([[V[0],V[3]+1j*V[4],V[5]+1j*V[6]],[V[3]-1j*V[4],V[1],V[7]+1j*V[8]],[V[5]-1j*V[6],V[7]-1j*V[8],V[2]]])
        V=c_linear_ground2.steady_rhos(deloval,delmval,0, 0,p)
        rho0_single2[:,:,ii,jj]=np.array([[V[0],V[3]+1j*V[4],V[5]+1j*V[6]],[V[3]-1j*V[4],V[1],V[7]+1j*V[8]],[V[5]-1j*V[6],V[7]-1j*V[8],V[2]]])

        #rho0[:,:,ii,jj]=c_linear_ground1.rho_broad_full(deloval,delmval, 0,p)
#rho0=rho0_single
...
# img1=ax.imshow((np.abs(rho0[1,1,:,:]+rho0[0,0,:,:]+rho0[2,2,:,:])),extent=(min(delamvals),max(delamvals),min(delaovals),max(delaovals)),aspect='auto',origin='lower')
...
for ii in range(3):
    for jj in range(3):
        ...
        img1=ax.imshow((np.angle(rho0_single1[ii,jj,:,:])),extent=(min(delamvals),max(delamvals),min(delaovals),max(delaovals)),aspect='auto',origin='lower')
...
for ii in range(3):
    for jj in range(3):
        ...
        ax=fig.add_subplot(3,3,pltnum)
        img1=ax.imshow((np.angle(rho0_single2[ii,jj,:,:])),extent=(min(delamvals),max(delamvals),min(delaovals),max(delaovals)),aspect='auto',origin='lower')
